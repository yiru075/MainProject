...
export default async function handler(req, res) {
    ...
    let query = 'health';
    if (category === 'social') {
      query = 'social OR wellbeing OR mindfulness OR community';
    }
  
    const encodedQuery = encodeURIComponent(query);
    const url = `https://api.eventfinda.com.au/v2/events.json?q=${encodedQuery}&free=1&point=${lat},${lng}&radius=20&rows=20&order=distance`;
